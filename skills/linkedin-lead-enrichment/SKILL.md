...
This mode checks `linkedin_connection_inventory` for the normalized profile URL before opening LinkedIn. If an existing record is found, stop and ask the user: "This lead already exists in the workflow inventory. What would you like me to do? Re-process — delete only this lead's existing candidate file/inventory record, then process it fresh. Skip — leave the existing record untouched." If they choose re-process, the agent may delete only the matching candidate markdown file and only the matching inventory row, then recreate and process that one profile. This duplicate re-process branch is the only approved AI deletion case.
